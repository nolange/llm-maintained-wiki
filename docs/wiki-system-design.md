...
- **Scripts are structure-agnostic.** No hardcoded paths. Scripts navigate via `_index.md`.
...
vault/
  ...
  wiki/                         ← flat, all articles at one level
    ...
    _index.md                   ← AI-maintained master index: all articles + tags
    _index_tsn.md               ← generated by filtering tag:tsn
    _index_finance.md           ← generated by filtering tag:finance
    ...
...
- Tag indexes (`_index_tsn.md` etc.) are generated by filtering frontmatter tags
- `_index.md` is the master map of all articles and their tags
...
- **Writes:** wiki articles, `_index.md`, `assets/links.md`, asset sidecars (directly, via tool use)
...
5.  Claude reads source files, writes wiki articles to `wiki/`, updates `_index.md` and `assets/links.md`
6.  Python rebuilds all tag index files from wiki/ frontmatter, updates the manifest, and moves session entries to `processed/`
...
- **Reads:** wiki articles, clustered by tags
...
| `/wiki-ask "question"` | Reads `_index.md` + relevant articles, answers in context |
...
- Not a RAG system — the AI navigates via `_index.md` and reads relevant docs directly
